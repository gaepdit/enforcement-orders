    ...
    IQueryableUserStore<ApplicationUser>
{
    ...
    private List<ApplicationUser> UserStore { get; } = UserData.GetUsers.ToList();
    private ICollection<IdentityRole<Guid>> Roles { get; } = UserData.GetRoles.ToList();
    private List<IdentityUserRole<Guid>> UserRoles { get; } = [];
    ...
    {
        var existingUser = await FindByIdAsync(user.Id.ToString(), cancellationToken).ConfigureAwait(false)
                           ?? throw new ArgumentException("User ID not found", nameof(user));
    ...
    public Task<ApplicationUser> FindByIdAsync(string userId, CancellationToken cancellationToken) =>
        Task.FromResult(UserStore.SingleOrDefault(u => u.Id.ToString() == userId));
    ...
    public Task AddToRoleAsync(ApplicationUser user, string roleName, CancellationToken cancellationToken)
    {
        ...
        if (!exists) UserRoles.Add(new IdentityUserRole<Guid> { RoleId = roleId.Value, UserId = user.Id });
